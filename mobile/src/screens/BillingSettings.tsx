...
type BatchItem={id:string;user_id:string|null;unit_label:string|null;payer_name:string;amount_cents:number;baseFeeCents?:number;extraChargeCents?:number;extraChargeDescriptions?:string[];manualOverride?:boolean;due_date:string;status:string;issues:string[];invoice_id:string|null};
...
export default function BillingSettings({navigation}:any){
 ...
 return (
  <>
    <ScrollView contentContainerStyle={[s.container,mobile&&s.containerMobile]}>
 ...
 <Text style={s.heading}>Histórico de competências</Text>
              ...
              {viewingItems.map(item=>{
                ...
                return <View key={item.id} style={s.itemRow}>
                  <View style={s.personContent}>
                    ...
                    <Text style={s.meta}>
                      {item.manualOverride?'Valor manual · ':item.extraChargeCents?`Taxa condominial ${money(item.baseFeeCents||0)} + Cobrança adicional (${item.extraChargeDescriptions?.join(', ')}) ${money(item.extraChargeCents)} = `:''}{money(item.amount_cents)}
                    </Text>
