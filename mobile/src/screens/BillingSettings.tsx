...
type Person={id:string;full_name:string|null;username:string;role:string;unit:string|null;unit_id?:string|null;is_unit_financial_responsible?:boolean;unit_type_name?:string|null;condominium_fee_cents?:number|null;billing_exempt?:boolean;preferred_due_day?:number};
...
// Uma unidade só pode gerar uma cobrança por competência. Quando mais de um
// morador cobrável da mesma unidade está cadastrado (dono + inquilino),
// só quem estiver marcado como responsável financeiro (Blocos e unidades >
// Definir financeiro) entra como candidato aqui — sem ninguém marcado, a
// unidade inteira fica de fora até o síndico definir, em vez de escolher
// "um qualquer" e arriscar gerar boleto real para a pessoa errada.
const dedupeByUnit=(eligible:Person[]):{people:Person[];needsFinancialResponsible:{unit:string;names:string[]}[]}=>{
  const byUnit=new Map<string,Person[]>();
  const withoutUnit:Person[]=[];
  for(const person of eligible){if(!person.unit_id){withoutUnit.push(person);continue;}const list=byUnit.get(person.unit_id)||[];list.push(person);byUnit.set(person.unit_id,list);}
  const people=[...withoutUnit];
  const needsFinancialResponsible:{unit:string;names:string[]}[]=[];
  for(const group of byUnit.values()){
    if(group.length===1){people.push(group[0]);continue;}
    const responsible=group.filter(p=>p.is_unit_financial_responsible);
    if(responsible.length===1){people.push(responsible[0]);continue;}
    needsFinancialResponsible.push({unit:group[0].unit||'Sem unidade',names:group.map(p=>p.full_name||p.username)});
  }
  return {people,needsFinancialResponsible};
};
...
export default function BillingSettings({navigation}:any){
 ...
 const [needsFinancialResponsible,setNeedsFinancialResponsible]=useState<{unit:string;names:string[]}[]>([]);
