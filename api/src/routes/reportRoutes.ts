...
const pushUsers=async(condominiumId:string,userIds:string[],title:string,body:string,data:Record<string,unknown>)=>{
	if(!userIds.length)return;
	const devices=await query<{fcm_token:string}>(`select distinct fcm_token from device_tokens where user_id = any($1::uuid[])`,[userIds]);
	const tokens=devices.rows.map(item=>item.fcm_token).filter(Boolean);
	if(!tokens.length)return;
	try{await sendPushNotification({tokens,title,body,data});}catch(error){console.warn('Failed to send report push notification',error);}
};
